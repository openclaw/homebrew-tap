...
targets.each do |os, arch|
  Homebrew::SimulateSystem.with(os:, arch:) do
    ...
    cases = { legacy: [], complete: companions }
    ...
    cases.each do |name, members|
      Dir.mktmpdir("crabbox-formula-install-") do |directory|
        ...
        expected.each { |file, contents| (source/file).write(contents) }
        ...
        if members.empty? || members == companions
          raise "Unexpected failure for #{os}/#{arch}/#{name}" if failed

          actual = destination.children.to_h { |file| [file.basename.to_s, file.read] }
