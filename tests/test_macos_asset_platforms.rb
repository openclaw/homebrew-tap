  ...
  paths = JSON.parse(output).map do |name, text|
    ...
    platforms.each do |os, arch|
      Homebrew::SimulateSystem.with(os:, arch:) do
        klass = Formulary.load_formula("peekaboo", path, text, "MacosAssets#{name}#{os}#{arch}",
        ...
        expected = "https://github.com/openclaw/Peekaboo/releases/download/v4.4.1/peekaboo-macos-#{target}.tar.gz"
        ...
        raise "Wrong version for #{name}/#{os}/#{arch}" if formula.version.to_s != "4.4.1"
        ...
        if name == "existing" && requirement.version != MacOSVersion.from_symbol(:sequoia)
